...
alter table private.checkout_settings
  add column if not exists dubai_shipping_fee numeric(12,2) not null default 15 check (dubai_shipping_fee >= 0),
  add column if not exists uae_other_shipping_fee numeric(12,2) not null default 25 check (uae_other_shipping_fee >= 0),
  add column if not exists shipping_country_code char(2) not null default 'AE';
...
begin
  ...
  v_country_code := upper(btrim(p_shipping_address->>'country_code'));
  v_city := lower(btrim(p_shipping_address->>'city'));

  if v_country_code <> v_settings.shipping_country_code then
    raise exception 'SHIPPING_DESTINATION_NOT_SUPPORTED';
  end if;
  ...
  if v_settings.shipping_enabled is true then
    if v_city in ('dubai', 'دبي') then
      v_shipping := v_settings.dubai_shipping_fee;
    else
      v_shipping := v_settings.uae_other_shipping_fee;
    end if;
